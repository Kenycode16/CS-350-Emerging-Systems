            ...
            for word in wordList:
                ...
                for char in word:
                    ...
                    for x in morse:
                        ...
                        if x == ".":
                            self.send("doDot")
                            self.send("doDot")  # dot.to(off)
                        elif x == "-":
                            self.send("doDash")
                            self.send("doDash")  # dash.to(off)

                        # Pause between dots/dashes:
                        if morseCounter < lenMorse:
                            self.send("doDDP")
                            self.send("doDDP")  # dotDashPause.to(off)
                        morseCounter += 1

                    # Pause between letters:
                if wordCounter < lenWord:
                    self.send("doLP")
                    self.send("doLP")  # letterPause.to(off)
                wordCounter += 1

                    # Pause between words:
            if wordsCounter < lenWords:
                    self.send("doWP")
                    self.send("doWP")  # wordPause.to(off)
            wordsCounter += 1
